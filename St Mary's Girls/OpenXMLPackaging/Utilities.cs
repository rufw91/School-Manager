...
namespace OpenXmlPackaging {
    public class Utilities {
        ...
        {
            return Utilities.ColumnNames.IndexOf(Utilities.GetColumnName(cellName)) + 1;
        }
        ...
        public static bool ValidateRangeReference(string rangeReference)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(rangeReference))
                    throw new ArgumentException("Invalid Reference");
                var references = rangeReference.Split(new[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        internal static string GetStartRefFromRange(string rangeReference)
        {
            var references = rangeReference.Split(new[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            if (references.Length == 1 || references.Length == 2)
            {
        ...
        internal static string GetEndRefFromRange(string rangeReference)
        {
            var references = rangeReference.Split(new[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            if (references.Length == 1 || references.Length == 2)
            {
