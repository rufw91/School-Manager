...
namespace OpenXmlPackaging
{
    ...
    public class Worksheet
    {
        ...
        {
            if (this.Cells.AllCells == null)
                return new DataTable();
            if (this.Cells.AllCells.Count == 0)
                return new DataTable();
            Utilities.ValidateRangeReference(Dimension);

            return Cells.ExportDataTable(Dimension, true);
        ...
        internal void Open(XmlReader reader, Dictionary<int, string> sharedStrings)
        {
            if (string.IsNullOrWhiteSpace(Dimension))
            {
                using (var rx = XmlReader.Create(_worksheetPart.GetStream(FileMode.Open, FileAccess.Read)))
                {
                    GetDimension(rx);
                }
            }
            Cells.Read(reader, sharedStrings);
        }
        private bool GetDimension(XmlReader reader)
        {
            try
            {
                while (reader.Read())
