...
namespace OpenXmlPackaging
{
    ...
    public class Cells : DataWriter
    {
        ...
        public DataTable ExportDataTable(string rangeReference, bool includeColumnNames)
        {
            ...
            string startReference, endReference;
            startReference = Utilities.GetStartRefFromRange(rangeReference);
            endReference = Utilities.GetEndRefFromRange(rangeReference);
            int startColumnIndex = Utilities.GetColumnIndex(startReference);
            int startRowIndex = Utilities.GetRowIndex(startReference);

            int endColumnIndex = Utilities.GetColumnIndex(endReference);
            int endRowIndex = Utilities.GetRowIndex(endReference);
        ...
        internal void AddCell(string cellName, string value)
        {
            int row = Utilities.GetRowIndex(cellName);
            // Get the current row
            var cellRow = AllCells.ContainsKey(row) ? AllCells[row] : null;
            ...
            // Create a cell
            var cell = new Cell(cellName) { Stylesheet = _styleSheet, Value = value };
        ...
        internal bool Read(XmlReader reader, Dictionary<int, string> sharedStrings)
        {
            try
            {
                while (reader.Read())
