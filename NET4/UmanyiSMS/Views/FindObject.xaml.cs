...
namespace UmanyiSMS.Views
{
    public partial class FindObject : CustomWindow, INotifyPropertyChanged
    {
        ...
        public FindObject()
        {
            selectedItems = new ObservableImmutableList<dynamic>();
            InitializeComponent();
            SearchText = "";
            PropertyChanged += (o, e) =>
                {
            ...
            AllItems = new ObservableImmutableList<dynamic>();
        ...
        private void Search()
        {
            ...
            dtGrid.Items.Filter = new Predicate<object>((item) =>
            {
                if (item != null)
                {
                    string st = item.GetType().GetProperty(SearchProperty).GetValue(item,null).ToString();
